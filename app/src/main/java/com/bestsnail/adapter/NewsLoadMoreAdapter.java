 ...
 */
public class NewsLoadMoreAdapter extends BaseRecyclerAdapter<NewsTable> {
    public static final int PULL_TO_LOAD_MORE = 0;
    public static final int LOADING = 1;
    public static final int NO_MORE = 2;
    private int mFooterStatus = PULL_TO_LOAD_MORE;

    public NewsLoadMoreAdapter(Context ctx, List<NewsTable> list) {
        super(ctx, list);
    }
    ...
    @Override
    public void bindData(RecyclerViewHolder holder, int position, NewsTable item) {
        //调用holder.getView(),getXXX()方法根据id得到控件实例，进行数据绑定即可
        holder.setText(R.id.tv_num, item.getNews_title());
        holder.setText(R.id.author, item.getNews_author());
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        holder.setText(R.id.time, simpleDateFormat.format(item.getNews_time()));
    }
