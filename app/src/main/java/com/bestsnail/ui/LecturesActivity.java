...
import com.bestsnail.adapter.LecturesMoreAdapter;
...
public class LecturesActivity extends AppCompatActivity {
    ...
    List<LecturesTable> mlist = new ArrayList<>();
    List<LecturesTable> listOld = new ArrayList<>();
    private int page = 1;
    ...
    private void initView() {
        ...
        //调用适配器加载资源
        mAdapter = new LecturesMoreAdapter(LecturesActivity.this, mlist);

        _mRecyclerView.setLayoutManager(new LinearLayoutManager(LecturesActivity.this));
        _mRecyclerView.setAdapter(mAdapter);
        mAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View itemView, int pos) {
                Intent intent = new Intent(LecturesActivity.this, LecturesInfoActivity.class);
        ...
        _mRecyclerView.addOnScrollListener(new FootLoadScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (isScrollEnd(recyclerView, newState)) {
                    page++;
                    ((LecturesMoreAdapter) mAdapter).setFooterStatus(LecturesMoreAdapter.LOADING);//更新正在拼命加载文字
                    HttpUtils httpUtils = new HttpUtils();
                    String url = GetHttp.getHttpConnetion() + "GetLectures";
                    ...
                    httpUtils.send(HttpMethod.POST, url, pre, new RequestCallBack<String>() {
                        ...
                        public void onSuccess(ResponseInfo<String> responseInfo) {
                            ...
                            if ("[]".equals(result)) {
                                ((LecturesMoreAdapter) mAdapter).setFooterStatus(LecturesMoreAdapter.NO_MORE);//更新文字没有更多了
                            } else {
                                Type type = new TypeToken<List<LecturesTable>>() {
                                }.getType();
                                Gson gson = GetGson.mGetGson();
                                List<LecturesTable> list = gson.fromJson(String.valueOf(result), type);
                                for (LecturesTable i :
                                        list) {
...
//                        _mSwipeRefresh.setRefreshing(true);
    ...
    public void firstLoad() {
        ...
        httpUtils.send(HttpMethod.POST, url, pre, new RequestCallBack<String>() {
            ...
            @Override
            public void onFailure(HttpException error, String msg) {
                Toast.makeText(LecturesActivity.this, "请检查网络", Toast.LENGTH_SHORT).show();
